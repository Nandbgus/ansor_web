<article class="prose lg:prose-xl mx-auto w-full">
        ...
        <div class="foto relative" style="background-image: url('<?= BASEURL ?>/img/blog/<?= $data['blog']['foto_blogs'] ?>'); background-size: cover; background-position: center; width: 100%; height: 400px;">
            <div class="absolute bottom-0 right-0 mb-4">
                <?php foreach ($data['blog']['kategories'] as $category) : ?>
                    <span class="mr-2 mb-2 px-2 py-1 bg-red-500 text-white rounded text-sm"># <?= htmlspecialchars($category['nama_kategori'], ENT_QUOTES, 'UTF-8') ?></span>
                <?php endforeach; ?>
            </div>
        </div>
